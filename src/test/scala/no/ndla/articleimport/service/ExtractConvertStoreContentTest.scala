...
class ExtractConvertStoreContentTest extends UnitSuite with TestEnvironment {
  ...
  test("That ETL extracts, translates and loads a node correctly") {
    ...
    when(extractConvertStoreContent.processNode(nodeId2, ImportStatus(Seq(), Seq(), Set(nodeId))))
    ...
    status should equal(
      ImportStatus(List(s"Successfully imported node $nodeId: 1"), Seq.empty, Set(nodeId, nodeId2), Some(sampleArticle.id), false))
  ...
  test("That ETL returns a list of visited nodes") {
    ...
    status should equal(
      ImportStatus(List(s"Successfully imported node $nodeId: 1"), Seq(), Set("9876", nodeId), Some(sampleArticle.id), false))
  }
  ...
  test("Articles should be force-updated if flag is set") {
    ...
    when(extractConvertStoreContent.processNode(nodeId2, ImportStatus.empty(forceUpdate = true).addVisitedNode(nodeId)))
      .thenReturn(Try((sampleArticle, ImportStatus(Seq(), Seq(), Set(nodeId, nodeId2), Some(1), false, true))))
    when(draftApiClient.getArticleIdFromExternalId(any[String]))
      .thenReturn(Some(1: Long))
    ...
    status should equal(
      ImportStatus(List(s"Successfully imported node $nodeId: 1"), Seq(), Set(nodeId), Some(sampleArticle.id), false, true))
  ...
  test("Articles should not be force-updated if flag is not set") {
    ...
    when(
      ...
      .thenReturn(Try((sampleArticle, ImportStatus(Seq(), Seq(), Set(nodeId, nodeId2), Some(1), false, false))))
    when(draftApiClient.getArticleIdFromExternalId(any[String]))
      .thenReturn(Some(1: Long))
    when(draftApiClient.getContentByExternalId(any[String]))
      .thenReturn(Some(sampleArticle))
