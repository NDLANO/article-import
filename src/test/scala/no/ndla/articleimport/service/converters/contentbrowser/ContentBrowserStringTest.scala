...
class ContentBrowserStringTest extends UnitSuite with TestEnvironment {
  ...
  val altValidContentBrowser = ContentBrowserString(LeftMarginContentBrowser, "nb")
  val alt2ValidContentBrowser = ContentBrowserString(RightContentBrowser, "nb")
  val invalidContentBrowser = ContentBrowserString(contentString.substring(1), "nb")
  ...
  test("That getStartEndIndex returns the correct start and end indexes") {
    ...
    ContentBrowserString("junk junk" + LeftMarginContentBrowser + "also junk", "nb").StartEndIndex should equal((9, 412))
  }
