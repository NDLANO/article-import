...
class ContentBrowserStringTest extends UnitSuite with TestEnvironment {
  val nodeId = "1234"
  val altText = "Jente som spiser melom. Grønn bakgrunn, rød melon. Fotografi."

  def htmlDocWithContentBrowserString(cbString: String): Element =
    stringToJsoupDocument(s"<section>$cbString</section>")
  ...
  val LeftMarginContentBrowser =
    s"[contentbrowser ==nid=$nodeId==imagecache=Hoyrespalte==width=180==alt=Enkle munnbind. Foto.==link===node_link=1==link_type=link_to_content==lightbox_size===remove_fields[76661]=1==remove_fields[76663]=1==remove_fields[76664]=1==remove_fields[76666]=1==insertion===link_title_text=Enkle munnbind.==link_text=Enkle munnbind.==text_align=right==css_class===css_class=contentbrowser contentbrowser_margin_left]"

  val RightContentBrowser =
    "[contentbrowser ==nid=24683==imagecache=Hoyrespalte==width===alt=tikroning, mynt. Foto.==link===node_link=1==link_type=link_to_content==lightbox_size===remove_fields[76661]=1==remove_fields[76666]=1==insertion===link_title_text= ==link_text= ==text_align=left==css_class===css_class=contentbrowser contentbrowser_margin_right]"
  val validContentBrowser = ContentBrowserString(htmlDocWithContentBrowserString(contentString), "nb")

  val altValidContentBrowser =
    ContentBrowserString(htmlDocWithContentBrowserString(LeftMarginContentBrowser), "nb")
  val alt2ValidContentBrowser = ContentBrowserString(htmlDocWithContentBrowserString(RightContentBrowser), "nb")

  val invalidContentBrowser =
    ContentBrowserString(htmlDocWithContentBrowserString(contentString.substring(1)), "nb")
  ...
  test("That getStartEndIndex returns the correct start and end indexes") {
    validContentBrowser.StartEndIndex should equal((9, 391))
    ContentBrowserString(htmlDocWithContentBrowserString("Innhold! " + contentString + "also junk"), "nb").StartEndIndex should equal(
      (18, 400))

    altValidContentBrowser.StartEndIndex should equal((9, 412))
    ContentBrowserString(htmlDocWithContentBrowserString("junk junk" + LeftMarginContentBrowser + "also junk"), "nb").StartEndIndex should equal(
      (18, 421))
  }
