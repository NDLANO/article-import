...
class HtmlTagGeneratorTest extends UnitSuite with TestEnvironment {
  ...
  test("A correctly formatted figure tag is returned") {
    val figureString: String =
      HtmlTagGenerator.buildEmbedContent(sampleDataAttributes)
    val expected =
      s"""<$ResourceHtmlEmbedTag data-caption="Sample image" data-resource="image" data-url="http://localhost/1" />"""

    figureString should equal(expected)
  }

  test("Correctly formatted content-link embed") {
    val contentLinkString: String =
      HtmlTagGenerator.buildEmbedContent(sampleContentLink)
    val expected =
      s"""<$ResourceHtmlEmbedTag data-content-id="2" data-link-text="http://localhost/2" data-open-in="new-context" data-resource="content-link" />"""
