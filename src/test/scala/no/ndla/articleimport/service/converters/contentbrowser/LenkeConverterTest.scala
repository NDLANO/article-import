...
class LenkeConverterTest extends UnitSuite with TestEnvironment {
  ...
  override def beforeEach = {
    ...
    when(extractService.getNodeEmbedMeta(nodeId)).thenReturn(Success(MigrationEmbedMeta(Some(linkUrl), Some(iframeEmbed))))
  }
  ...
  test("That LenkeConverter returns an a-tag if insertion method is 'link'") {
    val content = TestData.contentBrowserWithFields("nid" -> nodeId, "alt" -> altText, "link_text" -> " ", "insertion" -> "link")
  ...
  test("That LenkeConverter returns an a-tag if insertion method is 'lightbox_large'") {
    val content = TestData.contentBrowserWithFields("nid" -> nodeId, "alt" -> altText, "link_text" -> " ", "insertion" -> "lightbox_large")
    val expectedResult = """ <a href="https://www.youtube.com/watch?v=1qN72LEQnaU" rel="noopener noreferrer" target="_blank" title=""> </a>"""
  ...
  test("That LenkeConverter returns an a-tag if insertion method is 'collapsed_body'") {
    val content = TestData.contentBrowserWithFields("nid" -> nodeId, "alt" -> altText, "link_text" -> " ", "insertion" -> "collapsed_body")
