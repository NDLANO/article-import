...
class LenkeConverterTest extends UnitSuite with TestEnvironment {
  ...
  test("LenkeConverter should use url from embedCode if url is undefined") {
    ...
    when(extractService.getNodeEmbedMeta(nodeId))
      .thenReturn(Success(MigrationEmbedMeta(None, Some("<script src='http://infogr.am'></script>"))))
    val Success((result, _, errors)) =
      LenkeConverter.convert(content, ImportStatus.empty)

    result should equal(
      s"""<$ResourceHtmlEmbedTag data-resource="${ResourceType.ExternalContent}" data-url="http://infogr.am" />""")
  }
}
