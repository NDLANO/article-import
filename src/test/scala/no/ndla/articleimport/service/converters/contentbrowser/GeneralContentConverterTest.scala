...
class GeneralContentConverterTest extends UnitSuite with TestEnvironment {
  ...
  val sampleNodeToConvert = NodeToConvert(Seq(ArticleTitle("title", "en")), Seq(), "publicdomain", Seq(), Seq(), "fagstoff", "fagstoff", new Date(0), new Date(1), ArticleType.Standard, Seq.empty)
  ...
  test("That GeneralContentConverter inserts the content in language before languageless") {
    ...
    when(draftApiClient.getContentByExternalId(nodeId)).thenReturn(Some(sampleArticle))
    val Success((result, requiredLibraries, status)) = generalContentConverter.convert(content, ImportStatus.empty)
